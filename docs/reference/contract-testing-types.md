...
All four modes produce the same `ContractReport` (JSON / JUnit / HTML via the [CLI](../cli/contract-testing.md#reports)), and results can be recorded per pacticipant + version for the [`can-i-deploy` gate](../cli/contract-testing.md#can-i-deploy-the-deployment-gate).
...
- **`api-spector contract can-i-deploy`** reads those recorded results as a deployment gate (fail-closed), and **`contract report`** renders the compatibility dashboard.
...
| Dashboard / matrix | Hosted web UI | HTML export, or the read-only served dashboard (`contract report --serve`, docker-ready) |
...
| Webhooks (new contract triggers provider CI) | Built in | Built in: outbound webhooks fired by the served dashboard (`contracts/webhooks.json`) |
