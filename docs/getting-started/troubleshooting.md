...
`api-spector ui` launches the desktop app through the `electron` npm package. That package downloads a ~100 MB platform binary from GitHub Releases during `npm install` — and on locked-down or corporate machines, that download or its extraction is the thing that breaks. The CLI subcommands (`run`, `mock`, `record`, `contract`, `wsdl`) never need this binary and keep working regardless.
...
If IT policy keeps blocking downloads into `node_modules`, the most robust setup is to split UI and CLI:
