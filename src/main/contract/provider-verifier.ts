...
export function validateRequestAgainstSpec(
  ...
  if (req.body.mode === 'json' && req.body.json?.trim()) {
    const requestBody = resolveSchema(spec, (operation as any)['requestBody']) as Record<string, unknown> | null
  ...
  const parameters = resolveSchema(spec, (operation as any)['parameters'] ?? []) as unknown[]
