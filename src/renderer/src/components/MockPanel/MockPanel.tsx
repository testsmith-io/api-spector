        ...
        {recorderOpen && !recorderRunning && (
          <div className="px-3 pb-3 flex flex-col gap-2">
            <div className="flex flex-col gap-1">
              <label className="text-[10px] text-surface-500">{t('Upstream URL')}</label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] text-surface-500">{t('Local port')}</label>
              <input
                type="number"
                value={recorderPort}
...
{recorderError && (
        ...
        {recorderOpen && recorderRunning && (
          <div className="px-3 pb-2 text-[10px] text-surface-400">
            {t('Recording →')} {recorderUpstream}
            <span className="ml-2 text-surface-600">{t('(open panel to manage)')}</span>
          </div>
        )}
      </div>
      ...
      <div className="px-3 py-2 flex items-center justify-between border-b border-surface-800 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={t('Generate a mock server from a WSDL')}
          >
            {t('Import WSDL')}
          </button>
          <button
      ...
      {wsdlOpen && (
        <div className="px-3 py-2 flex flex-col gap-2 bg-surface-900/40 border-b border-surface-800 flex-shrink-0">
          ...
          <div className="flex gap-2">
            ...
            >
              {wsdlImporting ? t('Importing…') : t('Generate mock')}
            </button>
            <button
          ...
          <p className="text-[10px] text-surface-600 leading-relaxed">
            {t('One dispatch route per service endpoint. SOAPAction (or operation element in the body) selects which response envelope is returned.')}
          </p>
        </div>
      )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-surface-400 font-mono">
                    :{mock.port} · {t(':count route|:count routes', { count: (mock.routes ?? []).length })}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    title={entry.running ? t('Stop') : t('Start')}
                  >
                    {entry.running ? '■' : '▶'}
                  </button>
                  {cloudEnabled() && (
                    <button
                      ...
                      title={t('Push to cloud (choose routes)')}
                    >
                      ☁
                    </button>
                  )}
