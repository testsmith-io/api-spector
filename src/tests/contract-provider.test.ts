...
describe('resolveSchema', () => {
  ...
  it('resolves a $ref to the target schema', () => {
    ...
    expect((resolved['properties'] as any)?.id?.type).toBe('integer')
  })
  ...
  it('resolves nested $refs recursively', () => {
    ...
    const resolved = resolveSchema(spec, obj) as Record<string, unknown>
    expect((resolved['properties'] as any)?.name?.type).toBe('string')
  })
...
describe('findOperation', () => {
  ...
  it('finds a POST operation', () => {
    ...
    expect(match).not.toBeNull()
    expect((match!.operation as any).operationId).toBe('createPet')
  })
